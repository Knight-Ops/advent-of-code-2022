    ...
    input
        ...
        .filter(|(idx, tree_height)| {
            if idx < &input.row_length {
                ...
                // Check Down
                let end_of_column =
                    idx % input.row_length + (input.row_length * (input.row_length - 1));
                distance = 1;
                loop {
                    if &&input.trees[idx + distance * input.row_length] >= tree_height {
                        break;
...
pub fn part1_confusing(input: &TreeFarm) -> usize {
    ...
    for x in 1..input.row_length - 1 {
        ...
        for y in 1..input.row_length {
            if input.trees[input.row_length * (input.row_length - 1) + x - y * input.row_length]
                > max_height
            {
                max_height = input.trees
                    [input.row_length * (input.row_length - 1) + x - y * input.row_length];
                set.insert((x, y));
            }
